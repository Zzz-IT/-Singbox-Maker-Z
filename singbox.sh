...
SCRIPT_DIR="$(cd -- "$(dirname -- "${SELF_SCRIPT_PATH}")" && pwd -P)"
...
# --- 运行时自愈：缺组件则全量修复（install.sh） ---
_run_install_script() {
    # 优先使用本地安装目录的 install.sh（减少网络依赖）
    if [[ -x "${INSTALL_DIR_DEFAULT}/install.sh" ]]; then
        bash "${INSTALL_DIR_DEFAULT}/install.sh"
        return $?
    fi
    ...
    printf '%s\n' "错误: 未找到 curl 或 wget，无法执行安装脚本完成自愈/更新。" >&2
    return 1
}
...
if [[ -f "${SCRIPT_DIR}/utils.sh" ]]; then
    ...
    printf '%s\n' "检测到核心组件缺失（utils.sh/lib），正在尝试自动修复..." >&2
    _run_install_script || exit 1
    # shellcheck source=/dev/null
    source "${INSTALL_DIR_DEFAULT}/utils.sh"
fi
...
_sb_kill_pidfile() {
    ...
    pid="$(_sb_read_pidfile "$pid_file")" || return 0
    if _sb_pid_is_running "$pid"; then
...
_stop_argo_tunnel() {
    ...
    local pid_file="/tmp/singbox_argo_${target_port}.pid"
    _sb_kill_pidfile "$pid_file"
    rm -f -- "$pid_file" "/tmp/singbox_argo_${target_port}.log" 2>/dev/null || true
}
...
EOF
            ...
            if [[ "$("${YQ_BINARY}" eval 'has("proxy-groups")' "$CLASH_YAML_FILE")" != "true" ]]; then
                "${YQ_BINARY}" eval '.proxy-groups = [{"name": "节点选择", "type": "select", "proxies": []}] | .proxy-groups style="block"' -i "$CLASH_YAML_FILE"
                modified=true
            else
            ...
            if [[ "$("${YQ_BINARY}" eval 'has("rules")' "$CLASH_YAML_FILE")" != "true" ]]; then
                "${YQ_BINARY}" eval '.rules = ["MATCH,节点选择"] | .rules style="block"' -i "$CLASH_YAML_FILE"
                modified=true
            fi

            # 2.4 【高阶修复】节点数据自动同步与缩进格式化
            local proxy_count=$("${YQ_BINARY}" eval '.proxies | length' "$CLASH_YAML_FILE")
            if [ "$proxy_count" -gt 0 ]; then
                # 动态提取所有节点的 name，赋值给策略组，并强制转换为换行缩进格式 (block)
                "${YQ_BINARY}" eval '(.proxy-groups[] | select(.name == "节点选择")).proxies = [.proxies[].name] | (.proxy-groups[] | select(.name == "节点选择")).proxies style="block"' -i "$CLASH_YAML_FILE"
            else
                # 如果完全没节点了，直接置空
            ...
            if [ "$modified" = true ]; then
                _info "已自动修复 clash.yaml 结构并同步节点列表。"
            fi
        fi
    fi
}
...
_update_script() {
    ...
    if _run_install_script; then
        _success "更新完成，请重新运行脚本。"
        exit 0
    fi

    _error "更新失败：请检查网络或稍后重试"
    return 1
}
